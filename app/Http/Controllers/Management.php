    ...
    private function checkAndSyncLowStockAlerts()
{
    ...
    foreach ($lowStockProducts as $product) {
        ...
        if ($existingAlert) {
            // If resolved, create a brand new pending alert instead of updating
            if ($existingAlert->status === 'resolved') {
                ...
                // Active alert exists — just update stock and message if quantity changed
            ...
            // No alert exists at all — create one
            try {
                $adminUser = UserManagement::where('role', 'admin')->first();
                StockReport::create([
                    'user_id'         => $adminUser ? $adminUser->id : 1,
                ...
                // Silently ignore race-condition duplicates caught by DB unique constraint
            }
        }
    }

    // Clean up alerts for products that are back in stock
