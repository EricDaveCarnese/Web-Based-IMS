...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .brand-icon { background: linear-gradient(180deg, rgb(15,43,61) 0%, rgb(25,110,114) 100%);
        ...
        .nav-menu button { display: flex; align-items: center; background-color: transparent;
        ...
        .nav-item:hover:not(.active) { background-color: rgba(60,130,110,0.35); color: white;
            height: 40px; border-left: 3px solid #ffd700; border-radius: 15px; }
        .nav-item.active button { background: linear-gradient(180deg,rgb(15,43,61) 0%,rgb(25,110,114) 100%);
            color: white; border-left: 3px solid #ffd700; border-radius: 15px; height: 40px; }
        .nav-item { height: 40px; transition: all 0.3s ease; }

        .main-content { background: linear-gradient(45deg,rgb(44,110,98) 20%,#144243 50%,rgba(60,130,110,0.35) 100%);
        ...
        .topheader { display: flex; justify-content: space-between; align-items: center;
            padding: 5px; background-color: white; border-bottom: 1px solid rgb(210,210,210);
            flex-wrap: wrap; gap: 15px; }
        .page-title { margin-left: 30px; color: rgb(44,110,98); }
        .dashboard { margin-bottom: 3px; font-weight: bold; }
        .dashboard-sub { color: gray; font-size: 14px; }
        .user-menu-container a { background: linear-gradient(180deg,rgb(15,43,61) 0%,rgb(25,110,114) 100%);
            height: 35px; padding-inline: 15px; display: flex; align-items: center;
            justify-content: center; border-radius: 20px; text-decoration: none; gap: 5px; color: white; }
        .user-menu { display: flex; align-items: center; gap: 20px; color: white; margin-right: 25px; flex-wrap: wrap; }
        .logout-btn { background: linear-gradient(180deg,rgb(15,43,61) 0%,rgb(25,110,114) 100%);
        ...
        .notification-bell { background: linear-gradient(180deg,rgb(15,43,61) 0%,rgb(25,110,114) 100%);
        ...
        .notification-badge { position: absolute; top: -5px; right: -5px; background: #dc3545;
            color: white; border-radius: 50%; width: 20px; height: 20px; font-size: 11px;
            display: flex; align-items: center; justify-content: center; font-weight: bold; }
        .notification-dropdown { position: absolute; top: 50px; right: 0; width: 380px;
        ...
        .notification-header { background: linear-gradient(135deg,rgb(15,43,61) 0%,rgb(25,110,114) 100%);
            color: white; padding: 12px 15px; font-weight: 600; }
        .notification-list { max-height: 350px; overflow-y: auto; }
        .notification-item { padding: 12px 15px; border-bottom: 1px solid #e2e8f0; }
        .notification-item.unread { background: #e8f5e9; border-left: 3px solid #28a745; }
        .notification-title { font-weight: 600; color: #1e3a38; margin-bottom: 5px;
            display: flex; justify-content: space-between; align-items: flex-start; }
        .notification-message { font-size: 12px; color: #6c757d; margin-bottom: 8px; line-height: 1.4; }
        .notification-time { font-size: 10px; color: #9ca3af; white-space: nowrap; margin-left: 8px; }
        .notification-buttons { display: flex; gap: 8px; margin-top: 8px; flex-wrap: wrap; }
        .btn-order { background: #28a745; color: white; padding: 4px 10px; border-radius: 15px; font-size: 11px; cursor: pointer; border: none; }
        .btn-read  { background: #6c757d; color: white; padding: 4px 10px; border-radius: 15px; font-size: 11px; cursor: pointer; border: none; }
        .btn-edit-damage { background: #fd7e14; color: white; padding: 4px 10px; border-radius: 15px; font-size: 11px; cursor: pointer; border: none; }
        .resolved-badge { background: #28a745; color: white; padding: 4px 10px; border-radius: 15px;
            font-size: 11px; display: inline-flex; align-items: center; gap: 5px; }
        .notification-footer { padding: 10px 15px; text-align: center;
        ...
        .options { display: flex; justify-content: space-between; align-items: center;
            gap: 20px; margin: 20px 50px; flex-wrap: wrap; }
        ...
        .search-input { flex: 1; padding: 12px 16px; border: 1px solid #e2e8f0;
        ...
        .search-btn { background: linear-gradient(180deg,rgb(15,43,61) 0%,rgb(25,110,114) 100%);
            border: none; color: white; width: 44px; height: 44px; border-radius: 50%;
            cursor: pointer; display: flex; align-items: center; justify-content: center; }
        .filter-dropdown { padding: 10px 16px; border: 1px solid #e2e8f0; border-radius: 25px;
            font-size: 13px; background: white; cursor: pointer; min-width: 140px; }
        .add-product-button { background: linear-gradient(180deg,rgb(15,43,61) 0%,rgb(25,110,114) 100%);
            border: none; color: white; font-weight: 600; font-size: 16px; padding: 10px 24px;
            cursor: pointer; border-radius: 25px; display: flex; align-items: center; gap: 10px; transition: all 0.3s ease; }
        .right { display: flex; align-items: center; gap: 15px; flex-wrap: wrap; }
        .recordcount { color: rgb(71,241,4); padding: 11px 18px; border-radius: 25px; font-size: 14px;
            font-weight: 600; display: inline-flex; align-items: center; gap: 8px;
            background: linear-gradient(180deg,rgb(15,43,61) 0%,rgb(25,110,114) 100%); }
        .count { font-weight: 600; color: white; }

        /* Table */
        .product-table { margin: 20px 50px; background: white; border-radius: 20px;
            padding: 1.5rem; box-shadow: 0 10px 30px rgba(0,0,0,0.08); }
        ...
        .record-table { width: 100%; border-collapse: collapse; font-size: 0.9rem; min-width: 1000px; }
        .record-table th, .record-table td { padding: 14px 12px; text-align: left; border-bottom: 1px solid #e2e8f0; }
        .record-table th { background: linear-gradient(180deg,rgb(15,43,61) 0%,rgb(25,110,114) 100%);
        ...
        .description-cell { max-width: 300px; min-width: 200px; white-space: normal; word-wrap: break-word; }
        .short-desc, .full-desc { display: block; }
        .full-desc { display: none; }
        .toggle-description { color: rgb(44,110,98); cursor: pointer; font-size: 12px; text-decoration: none; }
        ...
        .price-cell { font-weight: 600; color: #2c6e62; }
        .edit-button, .delete-button { padding: 6px 14px; border-radius: 20px; cursor: pointer;
            font-size: 12px; display: inline-flex; align-items: center; gap: 5px; border: none; transition: all 0.3s ease; }
        .edit-button   { background: linear-gradient(180deg,rgb(40,140,203) 0%,rgb(25,110,114) 100%); color: white; }
        .delete-button { background: linear-gradient(180deg,rgb(188,179,170) 0%,rgb(214,162,79) 100%); color: white; }

        /* Alerts */
        .alert-success, .alert-error { position: relative; padding: 12px 40px 12px 20px;
            border-radius: 8px; margin: 10px 50px; border-left: 4px solid; }
        ...
        .close-btn { position: absolute; top: 50%; right: 15px; transform: translateY(-50%);
        ...
        .autocomplete-price { font-size: 11px; color: #666; margin-top: 3px; }
        ...
        .modal { width: 90%; max-width: 550px; max-height: 90vh; overflow-y: auto;
        ...
        .modal-header { margin-bottom: 20px; color: rgb(151,205,200); }
        .modal-header h2 { display: flex; align-items: center; gap: 10px; font-size: 1.4rem; }
        .modal input, .modal textarea, .modal select {
            width: 100%; padding: 12px 16px; border: 1px solid #e2e8f0;
            border-radius: 25px; font-size: 14px; margin-bottom: 15px; background: white; box-sizing: border-box; }
        .modal textarea { height: 80px; resize: vertical; }
        .save-button, .cancel-button { width: 100%; padding: 12px; border-radius: 25px; cursor: pointer;
        ...
        .damage-notice { background: #fd7e14; color: white; padding: 12px;
            border-radius: 8px; margin-bottom: 15px; text-align: center; font-size: 13px; }
        .damage-highlight { border: 2px solid #fd7e14 !important; background-color: #fff8f0 !important; }

        /* Pagination */
        .custom-pagination { display: flex; justify-content: center; gap: 8px; margin-top: 20px; flex-wrap: wrap; }
        .custom-pagination a, .custom-pagination span { display: inline-flex; align-items: center;
            justify-content: center; min-width: 32px; height: 32px; text-decoration: none;
            font-size: 14px; border-radius: 4px; }
        .custom-pagination a { color: #4a5568; cursor: pointer; }
        .custom-pagination a:hover { background: #2c6e62; color: white; }
        .custom-pagination .page-active { background: #2c6e62; color: white; }

        /* Toast */
        .toast-message { position: fixed; bottom: 20px; right: 20px; color: white;
            padding: 12px 20px; border-radius: 8px; z-index: 1100; animation: slideIn 0.3s ease; }
        @keyframes slideIn { from{transform:translateX(100%);opacity:0} to{transform:translateX(0);opacity:1} }

        @media (max-width: 1000px) { .sidebar{width:90px;} .brand h2,.nav-menu button span{display:none;} .main-content{margin-left:90px;} }
        @media (max-width: 860px)  { .options{margin:20px;flex-direction:column;} .right{width:100%;justify-content:space-between;} .product-table{margin:20px;} }
        @media (max-width: 480px)  { .options{margin:15px;} .product-table{margin:15px;padding:0.8rem;} .modal{padding:15px;} }
    </style>
</head>
<body>
    <!-- SIDEBAR -->
    <div class="sidebar">
        ...
        <div class="nav-menu">
            ...
            <form action="{{ route('admin.products') }}" method="GET"><div class="nav-item active"><button><i class="fas fa-cubes"></i><span>Products</span></button></div></form>
    ...
    <div class="main-content">
            ...
            <div class="user-menu">
                ...
                <div class="notification-area">
                    ...
                    <div class="notification-dropdown" id="notificationDropdown">
                        ...
                        <div class="notification-list" id="notificationList"><div style="padding:15px;text-align:center;color:#999;">Loading...</div></div>
        ...
        <div class="options">
            ...
            <div class="right">
                ...
                <div class="recordcount"><span class="count">Total Products: </span><strong>{{ $products->total() }}</strong></div>
                <button id="open_modal" class="add-product-button"><i class="fas fa-plus"></i><span>Add Product</span></button>
            </div>
        </div>
        ...
        <div class="modal-container" id="modal_container">
            <div class="modal">
                <div class="modal-header"><h2><i class="fa-solid fa-box-open"></i> Add New Product</h2></div>
                <form method="POST" action="{{ route('admin.product.store') }}">
                    ...
                    <button class="save-button" type="submit"><i class="fa-solid fa-circle-check"></i> Save Product</button>
                </form>
        ...
        <!-- ===== EDIT PRODUCT MODAL =====
             KEY FIX: ALL fields are always present and always submitted.
             In damage mode we visually grey them out with CSS but keep them
             enabled so the browser includes their values in the POST.
             We use readonly on text/number/textarea, and a visual-only class
             on selects (since selects can't be truly readonly, we lock them via JS pointer-events). -->
        <div class="modal-container" id="edit_modal_container">
            <div class="modal">
                <div class="modal-header"><h2><i class="fa-solid fa-box-open"></i> Edit Product</h2></div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="description-cell">
                                <span class="short-desc">{{ Str::limit($product->description, 80) }}</span>
                            ...
                            <td>{{ $product->min_stock_level }} units</td>
                            <td>
                                ...
                                @elseif($product->quantity <= $product->min_stock_level)
                                    <span class="status-badge status-low-stock"><i class="fas fa-exclamation-triangle"></i> Low Stock</span>
                                @else
                                    <span class="status-badge status-in-stock"><i class="fas fa-check-circle"></i> In Stock</span>
                                @endif
                            </td>
                            <td>
    ...
    // We NEVER use .disabled — we use CSS + readonly so values still submit.
    ...
    // This is the critical function — locks all fields except quantity.
    ...
    window.markRead = function(id) {
        ...
        .then(function(d){
            if (d.success) {
                ...
                fetchNotifications(); // refresh bell
            } else {
    ...
    document.addEventListener('DOMContentLoaded', function(){
        renderPagination();
        fetchNotifications();
        setInterval(fetchNotifications, 30000); // auto-refresh every 30s
    });
    </script>
</body>
</html>