        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function renderNotificationDropdown(notifications) {
                ...
                for (var i = 0; i < notifications.length; i++) {
                    ...
                    html += '<div class="notification-item unread" data-id="' +
                        notif.id + '" style="border-left: 3px solid ' +
                        stockColor + ';">' + '<div class="notification-title"><strong>' +
                        escapeHtml(notif.product_name) + '</strong><span class="notification-time">' +
                        notif.time_ago + '</span></div>' + '<div class="notification-message">Reported by: ' +
                        escapeHtml(notif.user_name) + '<br>Current Stock: <strong style="color:' + 
                        stockColor + ';">' + 
                        notif.current_stock + '</strong> units (Min: ' + 
                        notif.min_stock_level + ')<br><small>' + 
                            escapeHtml(notif.message.substring(0, 100)) + 
                            (notif.message.length > 100 ? '...' : '') + '</small></div>' + '<div class="notification-buttons">' + 
                                actionButton + '<button class="btn-read" onclick="markAsRead(' + 
                                notif.id + ')"><i class="fas fa-check"></i> Mark Read</button></div>' + '</div>';
                }
                list.innerHTML = html;
            }
