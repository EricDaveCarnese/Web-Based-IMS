...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .main-content { background: linear-gradient(45deg, rgb(44, 110, 98) 20%, #144243 50%, rgba(60, 130, 110, 0.35) 100%); flex: 1; overflow-y: auto; margin-left: 280px; min-height: 100vh; }
        ...
        .logout-btn { background: linear-gradient(180deg, rgb(15, 43, 61) 0%, rgb(25, 110, 114) 100%); border: none; cursor: pointer; color: white; padding: 10px 22px; border-radius: 30px; display: flex; align-items: center; gap: 12px; font-size: 14px; font-weight: 600; }
        
        .notification-area { position: relative; display: inline-block; margin-right: 15px; }
        .notification-bell { background: linear-gradient(180deg, rgb(15, 43, 61) 0%, rgb(25, 110, 114) 100%); border: none; color: white; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 18px; }
        .notification-bell:hover { transform: scale(1.05); }
        .notification-badge { position: absolute; top: -5px; right: -5px; background: #dc3545; color: white; border-radius: 50%; width: 20px; height: 20px; font-size: 11px; display: flex; align-items: center; justify-content: center; }
        .notification-dropdown { position: absolute; top: 50px; right: 0; width: 380px; background: white; border-radius: 12px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.15); display: none; z-index: 1000; max-height: 500px; overflow: hidden; }
        .notification-dropdown.show { display: block; animation: slideDown 0.3s ease; }
        @keyframes slideDown { from { opacity: 0; transform: translateY(-10px); } to { opacity: 1; transform: translateY(0); } }
        .notification-header { background: linear-gradient(135deg, rgb(15, 43, 61) 0%, rgb(25, 110, 114) 100%); color: white; padding: 12px 15px; font-weight: 600; }
        ...
        .notification-title { font-weight: 600; color: #1e3a38; margin-bottom: 5px; display: flex; justify-content: space-between; }
        .notification-message { font-size: 12px; color: #6c757d; margin-bottom: 8px; }
        ...
        .search-input { flex: 1; padding: 12px 16px; border: 1px solid #e2e8f0; border-radius: 25px; font-size: 14px; outline: none; }
        .search-btn { background: linear-gradient(180deg, rgb(15, 43, 61) 0%, rgb(25, 110, 114) 100%); border: none; color: white; width: 44px; height: 44px; border-radius: 50%; cursor: pointer; }
        ...
        .filter-dropdown { padding: 10px 16px; border: 1px solid #e2e8f0; border-radius: 25px; font-size: 13px; background: white; cursor: pointer; min-width: 140px; }
        .clear-btn { background: linear-gradient(180deg, rgb(15, 43, 61) 0%, rgb(25, 110, 114) 100%); border: none; color: white; font-weight: 600; padding: 10px 24px; cursor: pointer; border-radius: 25px; display: flex; align-items: center; gap: 10px; }
        .right { display: flex; align-items: center; gap: 15px; }
        .recordcount { padding: 11px 18px; border-radius: 25px; font-size: 14px; font-weight: 600; background: linear-gradient(180deg, rgb(15, 43, 61) 0%, rgb(25, 110, 114) 100%); color: white; }
        .reports-container { margin: 20px 50px; background: white; border-radius: 20px; padding: 1.5rem; }
        .table-container { max-height: 600px; overflow-y: auto; overflow-x: auto; border-radius: 12px; }
        .report-table { width: 100%; border-collapse: collapse; font-size: 0.9rem; min-width: 1000px; }
        .report-table th, .report-table td { padding: 14px 12px; text-align: left; border-bottom: 1px solid #e2e8f0; }
        .report-table th { background: linear-gradient(180deg, rgb(15, 43, 61) 0%, rgb(25, 110, 114) 100%); color: white; position: sticky; top: 0; }
        ...
        .status-pending { background: #fff3cd; color: #856404; padding: 6px 12px; border-radius: 20px; font-size: 12px; display: inline-flex; align-items: center; gap: 6px; }
        .status-read { background: #17a2b8; color: white; padding: 6px 12px; border-radius: 20px; font-size: 12px; display: inline-flex; align-items: center; gap: 6px; }
        .status-resolved { background: #28a745; color: white; padding: 6px 12px; border-radius: 20px; font-size: 12px; display: inline-flex; align-items: center; gap: 6px; }
        .status-ordered { background: #28a745; color: white; padding: 6px 12px; border-radius: 20px; font-size: 12px; display: inline-flex; align-items: center; gap: 6px; }
        
        .action-buttons { display: flex; gap: 8px; flex-wrap: wrap; }
        .btn-edit-product { background: #fd7e14; color: white; border: none; padding: 6px 12px; border-radius: 20px; cursor: pointer; font-size: 12px; display: inline-flex; align-items: center; gap: 6px; }
        .btn-edit-product:hover { background: #e86c00; transform: translateY(-1px); }
        .btn-mark-read { background: #6c757d; color: white; border: none; padding: 6px 12px; border-radius: 20px; cursor: pointer; font-size: 12px; }
        .btn-mark-read:hover { background: #5a6268; }
        .btn-create-po { background: #28a745; color: white; border: none; padding: 6px 12px; border-radius: 20px; cursor: pointer; font-size: 12px; display: inline-flex; align-items: center; gap: 6px; }
        .btn-create-po:hover { background: #218838; transform: translateY(-1px); }
        .resolved-text { color: #28a745; font-size: 12px; font-weight: 600; display: inline-flex; align-items: center; gap: 5px; background: #d4edda; padding: 6px 12px; border-radius: 20px; }
        .ordered-text { color: #28a745; font-size: 12px; font-weight: 600; display: inline-flex; align-items: center; gap: 5px; background: #d4edda; padding: 6px 12px; border-radius: 20px; }
        
        .reporter-info { display: flex; flex-direction: column; gap: 5px; }
        .reporter-name { font-size: 14px; font-weight: 500; }
        .reporter-badge { display: inline-flex; align-items: center; gap: 5px; padding: 3px 8px; border-radius: 12px; font-size: 10px; font-weight: 600; width: fit-content; }
        .reporter-badge.damage { background: #fd7e14; color: white; }
        .reporter-badge.auto { background: #17a2b8; color: white; }
        .damage-note { margin-top: 5px; font-size: 11px; color: #fd7e14; display: flex; align-items: center; gap: 5px; }
        
        .alert-success { background: #d4edda; color: #155724; padding: 12px 50px; margin: 10px 50px; border-radius: 8px; position: relative; }
        .alert-error { background: #f8d7da; color: #721c24; padding: 12px 40px; margin: 10px 50px; border-radius: 8px; position: relative; }
        ...
        .custom-pagination { display: flex; justify-content: center; gap: 8px; margin-top: 20px; }
        .custom-pagination a, .custom-pagination span { min-width: 32px; height: 32px; display: inline-flex; align-items: center; justify-content: center; text-decoration: none; border-radius: 4px; }
        .custom-pagination a { color: #4a5568; }
        .custom-pagination a:hover { background: #2c6e62; color: white; }
        .page-active { background: #2c6e62; color: white; }
        
        @media (max-width: 1000px) { .sidebar { width: 90px; padding: 1rem 0.5rem; } .brand h2, .nav-menu button span { display: none; } .main-content { margin-left: 90px; } }
        @media (max-width: 860px) { .options { flex-direction: column; } .search-container { max-width: 100%; } }
    </style>
</head>
<body>
    <div class="sidebar">
        ...
        <div class="nav-menu">
            ...
            <form action="{{ route('admin.stock.reports') }}" method="GET"><div class="nav-item active"><button><i class="fa-solid fa-triangle-exclamation"></i><span>Stock Reports</span></button></div></form>
        </div>
    </div>
    ...
    <div class="main-content">
        ...
        <div class="options">
            ...
            <div class="right">
                ...
                <div class="recordcount">Total: {{ $reports->total() }}</div>
        ...
        <div class="reports-container">
            <div class="table-container">
                <table class="report-table">
                    <thead>
                        <tr><th>Date & Time</th><th>Reported By</th><th>Product & Stock Info</th><th>Report Status</th><th>PO Status</th><th>Actions</th></tr>
                    </thead>
                    <tbody>
                        @forelse($reports as $report)
                            @php
                                ...
                                if ($isDamageReport) {
                                    ...
                                    $reporterBadge = '<span class="reporter-badge damage"><i class="fas fa-exclamation-triangle"></i> Damage Report</span>';
                                } else {
                                    // All non-damage reports (low stock, auto alerts, user reports) show as System Auto Alert
                                    $reporterIcon = ' ';
                                    $reporterBadge = '<span class="reporter-badge auto"><i class="fas fa-bell"></i> Auto Alert</span>';
                            ...
                            <tr @if($report->status == 'pending' && !$isDamageReport)@endif>
                                ...
                                <td>
                                    <div class="reporter-info">
                                        <div class="reporter-name">{!! $reporterIcon !!} <strong>{{ $displayReporter }}</strong></div>
    ...
    {{ $damageQty }} unit(s)</div>
                        ...
                        @empty
                            <tr><td colspan="6" style="text-align:center; padding:40px;">No stock reports found</span></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="custom-pagination">
                {{ $reports->appends(request()->only(['search', 'status']))->links() }}
            </div>
        </div>
    </div>

    <!-- Hidden data for suggestions -->
    ...
    <script>
        ...
        // Get suggestions data from hidden element
